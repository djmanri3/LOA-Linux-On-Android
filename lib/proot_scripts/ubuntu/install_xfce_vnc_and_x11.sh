...
if [ $? != 0 ]
then
	echo -e ""$r"[ERROR]$n Generate script file of install packages in ubun failled!"
else
	echo -e ""$g"[OK]$n Generate script file of install packages in ubun"
fi

gen_startubun
if [ $? != 0 ]
then
	echo -e ""$r"[ERROR]$n Generate script file of power on ubun failled!"
else
	echo -e ""$g"[OK]$n Generate script file of power on ubun"
fi
...
if [ $? != 0 ]
then
	echo -e ""$r"[ERROR]$n Generate script file to start ubun vnc failled!"
else
	echo -e ""$g"[OK]$n Generate script file to start ubun vnc"
fi
...
"$g"x11$n:
1. Execute [x11dubun] to start x11 environment and proot distro
