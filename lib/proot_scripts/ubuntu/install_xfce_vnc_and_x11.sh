...
function gen_install {
	...
	echo "apt install -y xorg lightdm $ui_package pulseaudio sudo $vnc_package" >> $proot_folder/root/$ui_install
	echo "echo">> $proot_folder/root/$ui_install
...
function gen_startubunvnc {
	echo "pulseaudio --start --load='module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1' --exit-idle-time=-1" > $proot_out/vncubun
	echo "proot-distro login $distro_name --shared-tmp -- runuser -l $uname -c vncstart" >> $proot_out/vncubun
