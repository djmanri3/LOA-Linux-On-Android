...
function gen_install {
	...
	echo "apt install -y xorg sddm $ui_package pulseaudio sudo $vnc_package lxterminal neofetch" >> $proot_folder/root/$ui_install
	echo "echo">> $proot_folder/root/$ui_install
...
function gen_startpi {
	...
	echo "proot-distro login $distro_name --shared-tmp -- runuser -l $uname -c x11start" >> $proot_out/x11pi
